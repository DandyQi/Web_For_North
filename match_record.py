...
class MatchRecord:
	...
	def __init__(self, fname):
		...
		self.output = open('./result/output.txt', 'w')
		...
		for c_index, c_value in comp.iteritems():
			if c_value == 0:
				...
				for b_index, b_value in ban.iteritems():
					if (c_value == b_value) and (c_index not in self.company_matched) and (b_index not in self.bank_matched):
		...
		if comp:
			for b_index, b_value in borrow.iteritems():
				if b_value == 0:
					...
					for l_index, l_value in loan.iteritems():
						if (b_value == l_value) and (b_index not in self.company_matched) and (l_index not in self.company_matched):
			...
			for b_index, b_value in borrow.iteritems():
				if b_value == 0:
					...
					for l_index, l_value in loan.iteritems():
						if (b_value == l_value) and (b_index not in self.bank_matched) and (l_index not in self.bank_matched):
							self.bank_matched.append(b_index)
			...
			if c_value == 0:
				...
				for i in range(1, 5):
					for j in combinations(ban_cur_index, i):
						...
						if ban_cur[list(j)].sum() == c_value and (c_index not in self.company_matched) and (len(test) == 0):
			...
			if b_value == 0:
				...
				for i in range(1, 5):
					for j in combinations(comp_cur_index, i):
						...
						if comp_cur[list(j)].sum() == b_value and (b_index not in self.bank_matched) and (len(test) == 0):
...
if __name__ == '__main__':
    ...
    MR.main()
>>>>>>> 80805366553e89a6fa8a3aa2c7440e7b596495a6
